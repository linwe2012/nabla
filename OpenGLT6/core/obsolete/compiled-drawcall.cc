...
RenderContext* GetRenderContext()
{
	if (gResourceBuffer == nullptr) {
		gResourceBuffer = new RenderResource::resource_t[4096];
	}

	if (!current.Isinitialized()) {
		current.Reset(gResourceBuffer, 4096);
	}
	return &current;
}*/
...
void SetUniform(MaterialHandle md, float data) {
	//TODO(L) Write msg
...
void SetUniform(MaterialHandle md, int data) {
	NA_LEAVE_IF((void)0, GetMaterialDecriptor(md).type != MaterialType::kInt, "");
	SetUniformHelper(md, data);
	return;
}

void SetUniform(MaterialHandle md, glm::vec3 data) {
...
void SetUniform(MaterialHandle md, glm::mat4 data) {
